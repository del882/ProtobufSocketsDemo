...
namespace SocketServerTcpListener
{
    ...
    public class AsyncService
    {
        ...
        {
            byte[] byteAfter64 = Convert.FromBase64String(request);
            

            using (MemoryStream stream = new MemoryStream(byteAfter64))
            {
                var requestObj = Serializer.DeserializeWithLengthPrefix<Request>(stream, PrefixStyle.Base128);

                //var requestObj = Serializer.Deserialize<Request>(stream);
                ...
                //{
                //    Serializer.Serialize<Response>(responseStream, resp);
                ...
                {
                    Serializer.Serialize<Response>(responseStream, resp);
                    string stringBase64 = Convert.ToBase64String(responseStream.ToArray());
        ...
        private static string OriginalResponse(string request)
        {
